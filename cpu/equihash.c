...
void collide_1_3(bucket_t* dst, bucket_t* src, element_indice_t* old_indices, size_t step_index) {
    ...
    for(uint32_t i = 0; i < NUM_BUCKETS; ++i) {
        ...
        element_t* bucket_data = (element_t*)(((uint8_t*)bucket->data) + start_byte);
        element_t* next_bucket_data = (element_t*)(((uint8_t*)bucket->data) + last_byte);


        if(step_index % 2 == 1) {
            for(uint32_t j = 0; j < bucket->size; ++j) {
                uint32_t sub_index = mask_collision_byte_bits_even_sub_bucket((uint8_t*)bucket_data) % NUM_ELEMENTS_BYTES_PER_BUCKET;
                sub_buckets[sub_index][sub_bucket_sizes[sub_index]][0] = mask_collision_byte_bits_odd((uint8_t*)next_bucket_data);
            ...
            for(uint32_t j = 0; j < bucket->size; ++j) {
                uint32_t sub_index = mask_collision_byte_bits_odd((uint8_t*)bucket_data) % NUM_ELEMENTS_BYTES_PER_BUCKET;
                sub_buckets[sub_index][sub_bucket_sizes[sub_index]][0] = mask_collision_byte_bits_even((uint8_t*)next_bucket_data);
                sub_buckets[sub_index][sub_bucket_sizes[sub_index]][1] = j;
                bucket_data++;
                next_bucket_data++;
                sub_bucket_sizes[sub_index]++;
...
void collide_4_7(bucket_t* dst, bucket_t* src, element_indice_t* old_indices, size_t step_index) {
    ...
    double ttot = get_tttime() - t3;
    fprintf(stderr, "colliding %zu: %zu (%f %f %f)\n", step_index, indice_index, tsort, tcollide, ttot);
}
...
void collide_8(bucket_t* dst, bucket_t* src, element_indice_t* old_indices, size_t step_index) {
    ...
    for(uint32_t i = 0; i < NUM_BUCKETS; ++i) {
        ...
        for(uint32_t j = 0; j < bucket->size; ++j) {
            uint32_t sub_index = mask_collision_byte_bits_odd_sub_bucket((uint8_t*)bucket_data) % NUM_ELEMENTS_BYTES_PER_BUCKET;
            sub_buckets[sub_index][sub_bucket_sizes[sub_index]][0] = mask_collision_byte_bits_even((uint8_t*)next_bucket_data);
            sub_buckets[sub_index][sub_bucket_sizes[sub_index]][1] = j;
            bucket_data++;
            next_bucket_data++;
...
size_t produce_solutions(uint32_t* dst_solutions, bucket_t* src, element_indice_t** indices, size_t n_src_elements, const blake2b_state* digest) {
    ...
    for(uint32_t i = 0; i < NUM_BUCKETS; ++i) {
        ...
        for(uint16_t j = 0; j < bucket->size; ++j) {
            uint32_t sub_index = mask_collision_byte_bits((uint8_t*)bucket_data->digest, start_byte, start_rel_bit) % NUM_ELEMENTS_BYTES_PER_BUCKET;
            sub_buckets[sub_index][sub_bucket_sizes[sub_index]][0] = mask_collision_byte_bits_final((uint8_t*)bucket_data->digest, last_byte, last_rel_bit);
            sub_buckets[sub_index][sub_bucket_sizes[sub_index]][1] = j;
            bucket_data++;
            sub_bucket_sizes[sub_index]++;
        }
...
void equihash_init_buckets(bucket_t** src, bucket_t** dst, element_indice_t*** indices) {
    ...
    for(size_t i = 0; i < EQUIHASH_K-1; ++i) {
        (*indices)[i] = (element_indice_t*)calloc(NUM_VALUES + (NUM_VALUES >> 2), sizeof(element_indice_t));
    }
...
size_t equihash(uint32_t* dst_solutions, const blake2b_state* digest, bucket_t* src, bucket_t* dst, element_indice_t** indices) {
    ...
    size_t n_solutions = produce_solutions(dst_solutions, src, indices, n_current_values, digest);
