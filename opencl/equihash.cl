...
void
set_element_digest_index (__global element_t * dst, uint32_t digest_index)
{
...
uint32_t
mask_collision_bits (__global uint8_t * data, size_t bit_index)
{
...
uint32_t
mask_collision_bits_step0 (uint8_t * data, size_t bit_index)
{
  uint32_t n = ((*data << (bit_index)) & 0xff) << 12;
  n |= ((*(++data)) << (bit_index + 4));
  n |= ((*(++data)) >> (4 - bit_index));
...
void
memcpy_step0 (__global void *dest, void *src, size_t n)
{
  char *csrc = (char *) src;
...
void
memcpy_global2private (void *dest, __global void *src, size_t n)
{
...
void
xor_elements (__global uint8_t * dst, __global uint8_t * a,
	      __global uint8_t * b)
{
...
bucket_collide_and_hash (__global digest_t * dst_digests,
			 ...
			 __global volatile uint32_t * new_digest_index,
			 __global src_local_bucket_t *
			 src_local_buckets_raw
			 ...
			 )
{
  ...
  for (uint32_t a = get_local_id (0); a < bucket->size; a += 32)
	...
	mask_collision_bits (digest + last_byte_index,
			     last_bit_index) % NUM_INDICES_PER_BUCKET;
      (src_local_buckets +
  ...
  for (uint32_t i = get_local_id (0); i < NUM_INDICES_PER_BUCKET; i += 32)
      ...
      for (uint32_t a = 0; a < src_local_bucket_sizes[i]; ++a)
	{
	  ...
	  for (uint32_t b = a + 1; b < src_local_bucket_sizes[i]; ++b)
		...
		(__global uint8_t *) src_digests[el.digest_index];
	      uint32_t new_index =
		base_collision_bits ^ mask_collision_bits (el_digest +
							   byte_index,
							   bit_index);
	      if (new_index == 0)
		continue;
  ...
  for (size_t i = get_local_id (0); i < NUM_INDICES_PER_BUCKET; i += 32)
      ...
      for (size_t a = 0; a < src_local_bucket_sizes[i]; ++a)
	{
	  ...
	  uint32_t base_collision_bits =
	    mask_collision_bits (base_digest + byte_index, bit_index);
	  int has_dupe = 0;
	  for (size_t b = a + 1; b < src_local_bucket_sizes[i] && !has_dupe;
	       ++b)
	    {
	      element_t el = src_local_bucket->data[b];
	      __global uint8_t *el_digest =
		...
		mask_collision_bits (base_digest + byte_index, bit_index);
	      uint32_t bi =
		mask_collision_bits (el_digest + byte_index, bit_index);
	      if (ai == bi && ai != 0)
		{
  ...
  for (size_t i = 0; i < EQUIHASH_K - 1; ++i)
      ...
      for (size_t j = 0; j < (1 << i); ++j)
	{
	  ...
	  __global bucket_t *parent_bucket =
	    buckets + ((EQUIHASH_K - 2 - i) * NUM_BUCKETS) +
	    parent_bucket_index;
	  elements[(i + 1) * NUM_INDICES + 2 * j] = parent_bucket->data[a];
	  elements[(i + 1) * NUM_INDICES + 2 * j + 1] =
	    parent_bucket->data[b];
	}
    }
